...
class UsersMapperTest {
  ...
  void shouldInsertUser() {
    ...
    Optional<Users> insertedUser =
        usersMapper.selectOne(c -> c.where(username, isEqualTo(user.getUsername())));
  ...
  void shouldFindMultipleUsers() {
    ...
    var users =
        usersMapper.select(c -> c.where(username, isIn("user1", "user2")).orderBy(username));
